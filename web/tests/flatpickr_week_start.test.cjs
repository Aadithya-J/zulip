...
function set_week_starts_on(code) {
    initialize_user_settings({user_settings: {week_starts_on: code}});
}

run_test("explicit mappings", () => {
    set_week_starts_on(2); // Saturday
    assert.equal(get_first_day_of_week(), 6);

    set_week_starts_on(3); // Sunday
    assert.equal(get_first_day_of_week(), 0);

    set_week_starts_on(4); // Monday
